...
async def scheduletopublish_specific_date(
    uploadSession: UploadSession, upload: YoutubeUpload
):
...
async def bulk_scheduletopublish_specific_date(
    videos: list, upload: YoutubeUpload
) -> None:
...
async def bulk_privatedraft(videos: list, upload: YoutubeUpload) -> None:
    ...
    for video in videos:
        ...
        tasks.append(privatedraft(uploadSession=video, upload=upload))
        await asyncio.gather(*tasks)
...
async def bulk_instantpublish(videos: list, upload: YoutubeUpload) -> None:
    ...
    for video in videos:
        tasks = []
        tasks.append(instantpublish(uploadSession=video, upload=upload))
        await asyncio.gather(*tasks)
